...
sub checkOptions {
    my %opts;
    getopts('hc:q:o:n:L:I:S:f', \%opts);
    my ($help, $contig, $query, $outDir, $outName, $length, $identity, $gSize, $frag);
    ...
    if($opts{c}) {
        $contig = $opts{c};
        if( -e $contig) {
            print "Assembly contig sequence is: $contig\n";
        } else {
            print "The assembly sequence is not in the correct format or doesn't exist.\n";
            print "Make sure you provide the full path (/root/path/fastq_file).\n";
            help();
        }
    } else {
        print "No assembly sequence path argument given.\n";
        help();
    }
	...
	my $outName2 = `basename "$contig"`;
	$outName = $outName2."_LoTrac";
	chomp($outName);
    ...
    return ($help, $contig, $query, $outDir, $outName, $length, $identity, $gSize, $frag);
}
...
USAGE
LoTrac_target.pl -c <assembly sequence file: file path> -q <query sequence file: file path> -o <output directory name: string> -n <output name prefix: string> -S <genome size>  [OPTIONS]
              
    -h   print usage
    -1   assembly sequence file (including full path)
...
my ($help, $contig, $query, $outDir, $outName, $length, $identity, $gSize, $frag) = checkOptions( @ARGV );
...
if (glob("prodigal_$outName*")) {
    ...
    system("prodigal -c -f gff -i $contig -a PRE_$outName.faa -o prodigal_$outName.gff -d PRE_$outName.fasta");
...
if (glob("TEMP_velvet_nucl_blast_db*")) {
    ...
    system("makeblastdb -in $contig -dbtype nucl -out TEMP_velvet_nucl_blast_db");
}
...
foreach (@query_names) {
	...
	my $subject_seq = extractFastaByID($best_name,$contig);
	my $subject_length = fasta_seq_length($subject_seq);

	print "\ncontig name of best hit against the query sequence: $best_name\n";
	print "% identity of best hit against the query sequence: $best_iden\n";
	print "length of best hit against the query sequence: $best_len\n";
	print "query length: $query_length\n";
	print "subject contig length: $subject_length\n";

	#if ($best_iden >= 50 && $frag_length >= 0.50) {
	if ($best_iden >= $match_iden && $frag_length >= $length) {
		...
		if ($frag_end >= $subject_length) {
		    $frag_end = $subject_length;
		}

		open(my $fh, '>', 'TEMP_frwd_extract.bed') or die "Could not open file 'TEMP_frwd_extract.bed' $!";
		print $fh "$best_name\t$frag_start\t$frag_end\n";
		close $fh;
		my $extract_frag_frwd = `bedtools getfasta -fi $contig -bed TEMP_frwd_extract.bed -fo stdout`;
		...
		if ($frag_start >= $subject_length) {
                    $frag_start = $subject_length;
                }
		...
		my $extract_frag_rev = `bedtools getfasta -tab -fi $contig -bed TEMP_rev_extract.bed -fo stdout`;
		if ($extract_frag_rev) {
